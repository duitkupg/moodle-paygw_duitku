...
class gateway extends \core_payment\gateway {
	...
	public static function add_configuration_to_gateway_form(\core_payment\form\account_gateway $form): void {
		...
		$text_attributes ='size="40"'; //Same as css styling for forms

		$mform->addElement('text', 'apikey', get_string('apikey', 'paygw_duitku'), $text_attributes);
		$mform->setType('apikey', PARAM_TEXT);
		$mform->addHelpButton('apikey', 'apikey', 'paygw_duitku');

		$mform->addElement('text', 'merchantcode', get_string('merchantcode', 'paygw_duitku'), $text_attributes);
		$mform->setType('merchantcode', PARAM_TEXT);
		$mform->addHelpButton('merchantcode', 'merchantcode', 'paygw_duitku');

		$mform->addElement('text', 'expiry', get_string('expiry', 'paygw_duitku'), $text_attributes);
